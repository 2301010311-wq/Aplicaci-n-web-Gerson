...
import { Plus, Search, Download, Eye, Trash2, Calendar, ChevronLeft, ChevronRight } from "lucide-react"
...
export function GestionIngresos() {
  ...
  const [filtrados, setFiltrados] = useState<Ingreso[]>([])
  ...
  useEffect(() => {
    filtrarIngresos()
  }, [ingresos, searchTerm, filtroCategoria, filtroFecha])
  ...
  const filtrarIngresos = () => {
    ...
    if (filtroFecha.inicio) {
      resultado = resultado.filter(
        (i) => new Date(i.fecha_ingreso) >= new Date(filtroFecha.inicio)
      )
    }

    if (filtroFecha.fin) {
      resultado = resultado.filter(
        (i) => new Date(i.fecha_ingreso) <= new Date(filtroFecha.fin)
      )
    }
  ...
  const eliminarIngreso = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/finanzas/ingresos/${id}`, {
        method: "DELETE",
      })
  ...
  const total = filtrados.reduce((sum, i) => sum + i.monto, 0)
    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-2">
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="bg-[#2F2F2F] border-[#C9A227]">
              <DialogHeader>
                <DialogTitle className="text-[#EAEAEA]">Registrar Nuevo Ingreso</DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select value={formData.categoria} onValueChange={(value) => setFormData({ ...formData, categoria: value })}>
                    ...
                    <SelectContent className="bg-[#2F2F2F] border-[#C9A227]">
                      <SelectItem value="Ventas" className="text-[#EAEAEA]">Ventas</SelectItem>
                ...
                <div>
                  <Label className="text-[#EAEAEA]">Cliente</Label>
                  <Input
                ...
                <div>
                  ...
                  <Select value={formData.metodo_pago} onValueChange={(value) => setFormData({ ...formData, metodo_pago: value })}>
                    ...
                    <SelectContent className="bg-[#2F2F2F] border-[#C9A227]">
                      ...
                      <SelectItem value="Cheque" className="text-[#EAEAEA]">Cheque</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
      ...
      <Card className="bg-[#2F2F2F] border-[#C9A227] border-opacity-20 overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-[#1C1C1C]">
              <tr>
                ...
                <th className="px-4 md:px-6 py-3 md:py-4 text-left text-[#C9A227] font-semibold text-xs md:text-sm">Cliente</th>
                <th className="px-4 md:px-6 py-3 md:py-4 text-left text-[#C9A227] font-semibold text-xs md:text-sm">Método</th>
            ...
            <tbody className="divide-y divide-[#1C1C1C]">
              {filtrados.map((ingreso) => (
                <tr key={ingreso.id_ingreso} className="hover:bg-[#1C1C1C] transition-colors">
                  <td className="px-4 md:px-6 py-3 md:py-4 text-[#EAEAEA] text-xs md:text-sm">
                    {new Date(ingreso.fecha_ingreso).toLocaleDateString()}
                  </td>
                  <td className="px-4 md:px-6 py-3 md:py-4 text-green-400 font-semibold text-xs md:text-sm">
                    S/ {ingreso.monto.toFixed(2)}
                  </td>
                  <td className="px-4 md:px-6 py-3 md:py-4 text-[#EAEAEA] text-xs md:text-sm">{ingreso.descripcion}</td>
                  <td className="px-4 md:px-6 py-3 md:py-4 text-[#EAEAEA] text-xs md:text-sm">{ingreso.categoria}</td>
                  <td className="px-4 md:px-6 py-3 md:py-4 text-[#EAEAEA] text-xs md:text-sm">{ingreso.cliente || "-"}</td>
                  <td className="px-4 md:px-6 py-3 md:py-4 text-[#EAEAEA] text-xs md:text-sm">{ingreso.metodo_pago}</td>
        ...
        <div className="md:hidden space-y-3 p-3">
          {filtrados.map((ingreso) => (
            <div key={ingreso.id_ingreso} className="bg-[#1C1C1C] p-3 rounded-lg border border-[#C9A227] space-y-2">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-[#C9A227] font-semibold text-xs">Fecha: {new Date(ingreso.fecha_ingreso).toLocaleDateString()}</p>
                  <p className="text-green-400 font-bold text-sm mt-1">S/ {ingreso.monto.toFixed(2)}</p>
                </div>
                <Button
                  onClick={() => eliminarIngreso(ingreso.id_ingreso)}
                  size="sm"
              ...
              <div className="space-y-1 text-xs">
                <p className="text-[#EAEAEA]"><span className="text-[#C9A227]">Desc:</span> {ingreso.descripcion}</p>
                <div className="grid grid-cols-2 gap-2">
                  <p className="text-[#EAEAEA]"><span className="text-[#C9A227]">Cat:</span> {ingreso.categoria}</p>
                  <p className="text-[#EAEAEA]"><span className="text-[#C9A227]">Pago:</span> {ingreso.metodo_pago}</p>
                </div>
                {ingreso.cliente && <p className="text-[#EAEAEA]"><span className="text-[#C9A227]">Cliente:</span> {ingreso.cliente}</p>}
              </div>
            </div>
          ))}
        </div>
        ...
        {filtrados.length === 0 && (
          <div className="text-center text-[#EAEAEA] py-8 text-xs md:text-sm">No hay ingresos registrados</div>
        )}
          ...
          <div className="bg-[#1C1C1C] px-4 md:px-6 py-3 md:py-4 border-t border-[#C9A227] border-opacity-20">
            <div className="flex justify-end">
              <div className="text-right">
                <p className="text-[#C9A227] text-xs md:text-sm">Total Ingresos</p>
                <p className="text-[#EAEAEA] text-xl md:text-2xl font-bold">S/ {total.toFixed(2)}</p>
              </div>
            </div>
          </div>
        )}
